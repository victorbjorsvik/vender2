...
def leverandorer():
    ...
    levs = db.execute("SELECT * FROM leverandorer ORDER BY navn")
...
def kontakt():
    ...
    # Create dict from database
    contacts = db.execute("SELECT * FROM kontakt")
...
def rediger_leverandorer():
    ...
    leverandorer = db.execute("SELECT * FROM leverandorer ORDER BY navn")
...
def leggtil_leverandor():
    ...
    db.execute("INSERT INTO leverandorer (navn, orgnummer, telefon, epost, adresse, postnummer, poststed) VALUES (?, ?, ?, ?, ?, ?, ?)",
...
def slett_leverandor():
    ...
    db.execute("DELETE FROM leverandorer WHERE navn = ?", request.form.get("navn2"))
...
def leggtil_kontakt():
    ...
    # Insert new supplier into database
    db.execute("INSERT INTO kontakt (stilling, navn, telefon, epost) VALUES (?, ?, ?, ?)",
...
def slett_kontakt():
    ...
    # Delete current contact from DB
    db.execute("DELETE FROM kontakt WHERE navn = ?", request.form.get("navn2"))
    flash(request.form.get("navn2") + " ble fjernet", 'alert alert-danger mb-0 text-center')
